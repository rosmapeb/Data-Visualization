...
for (x in 1:nrow(categoria))
{
  ...
  municipioC<-suppressWarnings(data.frame(rownames(municipioC),as.numeric(as.character(as.matrix(municipioC)))))
  municipioP<-as.data.frame(t(as.matrix(municipioP[2:12])))
  municipioP<-suppressWarnings(data.frame(rownames(municipioP),as.numeric(as.character(as.matrix(municipioP)))))
...
```R
write.csv(envipeII, file = "/home/rosa/Descargas/BASE_DE_DATOS_ENVIPE_2017_en/tb/tb-envipeII.csv", row.names = TRUE)
```
